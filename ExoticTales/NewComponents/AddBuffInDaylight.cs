...
- The RemoveBuffInDaylight is almost a perfect mirror of the above, apart from how the Triggering Buff works (which is the same way, so it enables the buff).
- The condition of "daylight" is defined in the same simplified way used by Race Unleashed:
  > You are outside a "Dungeon" environment.
  > It is either "Morning" or "Day".
  > The Weather is "Clear".
  > Any "Evening" or "Night" time, any "Dungeon" and any Weather that is not "Clear" will count as "Low-Light" for purpose of this simplified workaround.
...
public class AddBuffInDaylight : UnitFactComponentDelegate, IWeatherChangeHandler, IGlobalSubscriber, ISubscriber, IAreaLoadingStagesHandler, IUnitBuffHandler
    {
        ...
        public void HandleBuffDidAdded(Buff buff)    // This activates the desired EffectBuff ALSO when a specific TriggeringBuff is applied to the character.
        {
            if (buff.Blueprint == this.TriggeringBuff && buff.Owner == base.Owner && this.m_AppliedBuff == null)
            {
                this.ActivateBuff();
            }
        }
        ...
        {
            if (buff.Blueprint == this.TriggeringBuff && buff.Owner == base.Owner && !UnitHelper.HasFact(base.Owner, this.TriggeringBuff))
            {
                this.DeactivateBuff();
                this.Check();
            }
